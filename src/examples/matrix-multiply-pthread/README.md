...
The multiplication is parallelized with pthread workers. The number of
iterations and number of CPU cores (threads) are passed on the command line.
...
```sh
./matrix-multiply <iterations> <num_cores>
```

Example:

```sh
./matrix-multiply 10 8
```
